...
| **`deepLinkUrl`**           | <code>string</code>                                 | A deep link URL to open when the notification is tapped. When set, tapping the notification will launch the app with an `ACTION_VIEW` intent containing this URL, which can be handled via the `appUrlOpen` event or `App.getLaunchUrl()` from `@capacitor/app`. When not set, the notification will simply launch the app's main activity. |                    | 8.1.0 |
